...
このフォークの LuxSabers 専用非公開デプロイ設定と運用手順は [deploy/luxsabers-social](deploy/luxsabers-social/README.md) にあります。ルートのクイックスタート設定ではなく、公開処理を既定で一時停止する独立した Compose プロジェクトを使用します。
