...
function html(body, title = 'LuxSabers Social') {
  return `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><link rel="stylesheet" href="/session/style.css"><main>${body}</main></html>`;
}
...
export async function buildApp(config, { enableProxy = true } = {}) {
  ...
  app.get('/session', async (_, reply) => reply.type('text/html').send(html('<h1>LuxSabers Social</h1><p>Publishing paused</p><p>Model budget: $0</p><p><a href="/en">Open workspace</a></p><form method="post" action="/session/logout"><button type="submit">Sign out</button></form>')));
